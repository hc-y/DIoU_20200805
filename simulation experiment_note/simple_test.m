...
for k=1:160
    ...
    a(k,1:4)=pred;
    loss=abs(a(k,1:4)-gt(1:4))+loss;  % 每次梯度更新之后, 计算pred与gt在[ctr_x,ctr_y,w,h]上的差距, 并进行了累加
end
for k=161:180
    gt_tblr=to_tblr(gt);
    pred_tblr=to_tblr(pred);
...
figure,plot(a(:,1),'r','LineWidth',1.5),title('x');  % pred的[ctr_x,ctr_y,w,h]值随着每次梯度更新之后的变化
