...
% compute the derivative of DIoU w.r.t. [ctr_x,ctr_y,w,h] of `pred`;
...
Cbox=cover(pred,gt);
C=(Cbox.b-Cbox.t)*(Cbox.b-Cbox.t)+(Cbox.r-Cbox.l)*(Cbox.r-Cbox.l);  % Cbox对角线长度的平方
CCw=Cbox.r-Cbox.l;
CCh=Cbox.b-Cbox.t;
if pred.t<gt.t
    Ct.dx=0;
...
d=(A(1)-B(1))*(A(1)-B(1))+(A(2)-B(2))*(A(2)-B(2));  % A的中心点与B的中心点之间的欧式距离 的平方
...
diou=dIOU(pred,gt);
p.dx=diou.dl+diou.dr;  % 暂时没有除以2
p.dy=diou.dt+diou.db;  % 暂时没有除以2
